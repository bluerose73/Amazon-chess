...
double tiao_can(double t1,double t2,double p1,double p2,double f,int turnID){//调参 
	double ret=0;
...
Move minMax(int depth,double alpha,double beta){
	...
	for (int i = 0; i < GRIDSIZE; ++i) {//枚举所有可能下法 
...
//prt_map();
	// 做出决策（你只需修改以下部分）
	Move ans;
	for(maxDepth=1;maxDepth<=2;maxDepth++){//迭代加深搜索 
